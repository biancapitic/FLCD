...
class Scanner:
    def __init__(self, fileName):
        ...
        self.__operators = []
        self.__pif = []
        self.__faIdentifiers = FiniteAutomata('input/fa_identifiers.in')
        self.__faIntegerConst = FiniteAutomata('input/fa_integerConst.in')
    ...
    def __isConstant(self, token):
        if self.__faIntegerConst.isSequenceAccepted(token) or re.search('^"[a-zA-Z0-9\.\\+\*\?\=\!\|\:<\->;, ]*"$', token):
            return True
        return False

    def __isIdentifier(self, token):
        if self.__faIdentifiers.isSequenceAccepted(token):
            return True
        return False
